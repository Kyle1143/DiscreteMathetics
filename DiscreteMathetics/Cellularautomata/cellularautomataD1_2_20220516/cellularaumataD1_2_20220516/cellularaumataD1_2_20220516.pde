...
int[] init_cells = {1,0,1,0,0,0,0,1,0,1,1,1,0,0,0,1,1,1,0,0,1,0,1,0,0,0,0,1,0,1,1,1,0,0,0,1,1,1,0,0};
...
void draw()  {
  ...
  for (int i = 1; i < cur_cells.length-2; i++) {
    ...
    int leftmiddle = cur_cells[i];
    int rightmiddle = cur_cells[i+1];
    int right  = cur_cells[i+2];
    
    // The transformation rule is described in a separate function.
    // Temporarily saves the new state (color) in the new array
    new_cells[i] = apply_rule(left,leftmiddle,rightmiddle,right);
  }
...
class Rule {
  ...
  // Contructor
  Rule(String name, int left, int lmid, int rmid, int right, int newstat) {
    label=name;
    state_left=left;
    state_lmid=lmid;
    state_rmid=rmid;
  ...
  void draw() {
    ...
    draw_one_cell(posx, row_y, state_lmid, false);
    draw_one_cell(posx+cellsize, row_y, state_rmid, false);
    draw_one_cell(posx+cellsize+cellsize, row_y, state_right, false);
    draw_one_cell(posx, posy, state_new, true);
  }
 }
 
 // Here we have 16 rules.
 void init_rules(){
    int count = 0;

    rule = new Rule[16];
    rule[0]=new Rule("Rule 01", 0,0,0,0,0);
    rule[1]=new Rule("Rule 02", 0,0,0,1,1);
    rule[2]=new Rule("Rule 03", 0,0,1,0,1);
    rule[3]=new Rule("Rule 04", 0,0,1,1,0);
    rule[4]=new Rule("Rule 05", 0,1,0,0,0);
    rule[5]=new Rule("Rule 06", 0,1,0,1,0);
    rule[6]=new Rule("Rule 07", 0,1,1,0,0);
    rule[7]=new Rule("Rule 08", 0,1,1,1,0);
    rule[8]=new Rule("Rule 09", 1,0,0,0,0);
    rule[9]=new Rule("Rule 10", 1,0,0,1,0);
    rule[10]=new Rule("Rule 11", 1,0,1,0,0);
    rule[11]=new Rule("Rule 12", 1,0,1,1,0);
    rule[12]=new Rule("Rule 13", 1,1,0,0,0);
    rule[13]=new Rule("Rule 14", 1,1,0,1,0);
    rule[14]=new Rule("Rule 15", 1,1,1,0,0);
    rule[15]=new Rule("Rule 16", 1,1,1,1,0);
...
int apply_rule (int a, int b, int c, int d) {
    int result=0;
    for (int i = 0; i < rule.length; i++) {
      if (rule[i].state_left == a && rule[i].state_lmid == b && rule[i].state_rmid == c && rule[i].state_right == d){
