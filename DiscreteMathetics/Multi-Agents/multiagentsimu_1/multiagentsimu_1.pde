...
int Max_Human = 50; //シミュレータに登場させる人間の数
int base_time = 0; //シミュレーション開始時間
...
void draw() {
 ...
 int times = millis() - base_time;
 int season_count = 0;
  
 if(times > 8000){
   base_time = millis(); //時間を初期化
   season_count ++;
 }
 
 if((times>0) & (times<2000)){ //春
     // 背景を変更
   //makeground_s();
      background(#FFD6DE);
     //食べ物を発生
   if(int(random(1000)) < 40){
     MAP[int(random(Length))][int(random(Length))] = 2;
   }
 }
  else if((times>2000) & (times<4000)){ //夏
     // 背景を変更
   //makeground_sm();
      background(#D6FFE0);
     //食べ物を発生
  if(int(random(1000)) < 80){
    MAP[int(random(Length))][int(random(Length))] = 2;
  }
 }
 else if((times>4000) & (times<6000)){ //秋
     // 背景を変更
   background(#FFF4D6);
     //食べ物を発生
   if(int(random(1000)) < 100){
     MAP[int(random(Length))][int(random(Length))] = 2;
   }
 }
 else if((times>6000) & (times<8000)){ //冬
     // 背景を変更
    background(#D7D6FF);
    //makeground_w();
     //食べ物を発生
